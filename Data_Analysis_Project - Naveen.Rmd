...
```{r}
add_mod_1st = lm(log(SalePrice) ~  (MSSubClass + MSZoning  +GrLivArea + TotalBsmtSF + OverallQual+ YearBuilt+ FullBath + YearRemodAdd + Neighborhood +TotRmsAbvGrd+ExterQual +GarageArea), data = house_trn)
...
```{r}
bptest(add_mod_1st)
shapiro.test(resid(add_mod_1st))

```

```{r}
int_mod_1st = lm(log(SalePrice) ~  (MSSubClass + MSZoning + log(GrLivArea) + TotalBsmtSF + OverallQual+ OverallCond  + YearBuilt+  FullBath + 
YearRemodAdd + YearBuilt:YearRemodAdd + OverallCond:YearBuilt + Neighborhood + TotRmsAbvGrd + ExterQual + (GarageArea)), data = house_trn)
...
```{r}
bptest(int_mod_1st)
shapiro.test(resid(int_mod_1st))
```


```{r}
n = resid(int_mod_1st)
BIC_model_1st = step (int_mod_1st,k=log(nrow(house_trn)),trace=0)

summary(BIC_model_1st)
```


```{r}
bptest(BIC_model_1st)
shapiro.test(resid(BIC_model_1st))
```
...
int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+OverallQual+ YearBuilt+ YearRemodAdd + Neighborhood +log(TotRmsAbvGrd)+ExterQual)^2, data = house_trn)

# big_mod_cd = cooks.distance(int_add_mod)
# 
#   
# int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+OverallQual+YearBuilt + YearRemodAdd + Neighborhood +log(TotRmsAbvGrd)+ExterQual)^2, data = house_trn  , subset = big_mod_cd < 4 / length(big_mod_cd))
#   
# (RMSE_Train = sqrt(mean(resid(int_add_mod) ^ 2)))
# 
...
# 
# (RMSE_test = sqrt(mean((log(house_tst$SalePrice) - predict(int_add_mod,house_mod)) ^ 2)))
# 
#   
 
#int_add_mod = lm(log(SalePrice) ~ OverallQual + YearBuilt + YearRemodAdd + TotalBsmtSF + X1stFlrSF + log(GrLivArea) + Neighborhood  + FullBath + TotRmsAbvGrd + GarageArea + Foundation  + GarageType , data = house_trn)

summary(int_add_mod)

par(mfrow = c(1, 2))
plot(fitted(int_add_mod), resid(int_add_mod), col = "grey", pch = 20,
    xlab = "Fitted", ylab = "Residuals", main = "Data from Model 2")
abline(h = 0, col = "darkorange", lwd = 2)

qqnorm(resid(int_add_mod), main = "Normal Q-Q Plot", col = "darkgrey")
qqline(resid(int_add_mod), col = "dodgerblue", lwd = 2)

library(lmtest)
bptest(int_add_mod)
shapiro.test(resid(int_add_mod))
...
interation_model = lm(log(SalePrice) ~ (OverallQual + YearBuilt + YearRemodAdd + TotalBsmtSF + X1stFlrSF + log(GrLivArea) + Neighborhood  + FullBath + TotRmsAbvGrd + GarageArea  )^2, data = house_trn)

#+ MSSubClass +  Foundation  + GarageType
BIC_model = step (interation_model,k=log(nrow(house_trn)),trace=0)
...
```{r,eval=FALSE}
par(mfrow = c(1, 2))
plot(fitted(BIC_model), col = "grey", pch = 20,
    xlab = "Fitted", ylab = "Residuals", main = "Data from Model 2")
abline(h = 0, col = "darkorange", lwd = 2)
...
pval_results = data.frame(pval_models, pval_fit1, pval_fit2)
colnames(pval_results) = c("Train_RMSE", "Test_RMSE", "R_Squared")
knitr::kable(models)
```
