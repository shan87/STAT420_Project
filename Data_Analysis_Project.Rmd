...
int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+OverallQual+I(YearBuilt)^2+ YearRemodAdd + Neighborhood +log(TotRmsAbvGrd)+ExterQual)^2, data = house_trn)

big_mod_cd = cooks.distance(int_add_mod)

int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+OverallQual+I(YearBuilt)^2 + YearRemodAdd + Neighborhood +log(TotRmsAbvGrd)+ExterQual)^2, data = house_trn  , subset = big_mod_cd < 4 / length(big_mod_cd))
