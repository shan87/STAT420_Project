...
Now the data looks fine. Lets try to build a additive model with all variable as initial model to do a BIC search on it to find a smaller models with appropriate predictiors.

```{r}
for (col in colnames(train)){
   if(is.numeric(train[,col])){
       if( abs(cor(train[,col],train$SalePrice)) > 0.3){
           print(col)
