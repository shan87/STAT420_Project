...
house_data$TotalBsmtSF[which(is.na(house_data$TotalBsmtSF))] = 
  mean(house_data$TotalBsmtSF,na.rm=T)
...
house_trn$GarageArea= as.integer(house_trn$GarageArea)
...
#View(house_trn)
#int_add_mod = lm(log(SalePrice) ~ OverallQual+ GrLivArea +YearBuilt, data = house_trn  , subset = house_trn$SalePrice < 400000)

##Working for BP

#int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+I(FullBath)^2 +TotRmsAbvGrd+GarageCars)^2, data = house_trn  , subset = (house_trn$GrLivArea < 4000 & house_trn$SalePrice < 400000))

##improved BP and shapiro

#int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+YearBuilt+FullBath +TotRmsAbvGrd+GarageCars)^2, data = house_trn)

 int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+OverallQual+ I(YearBuilt)^2 + I(YearRemodAdd)^2 + Neighborhood +log(TotRmsAbvGrd)+ExterQual)^2, data = house_trn)
 

 big_mod_cd = cooks.distance(int_add_mod)
 
 int_add_mod = lm(log(SalePrice) ~  (log(GrLivArea)+OverallQual+ I(YearBuilt)^2 + I(YearRemodAdd)^2 + Neighborhood +log(TotRmsAbvGrd)+ExterQual)^2, data = house_trn  , subset = big_mod_cd < 4 / length(big_mod_cd))
...
interation_model = lm(log(SalePrice) ~ (OverallQual + YearBuilt + YearRemodAdd + TotalBsmtSF + X1stFlrSF + I(GrLivArea)^2 + Neighborhood  + FullBath + TotRmsAbvGrd + GarageArea  )^2, data = house_trn)
